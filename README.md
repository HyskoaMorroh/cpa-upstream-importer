...
```
输入框留空  →  只体检既有条目：重新探测 175 个，按结果重新生成
                headers / 代理 / 优先级 / 前缀。不会新增任何站。

有新账号    →  重新探测 175 个既有条目，并与本次新增的 3 行一起重新生成配置。
```
...
同一把 Key 提供多种协议。你这份配置 **175 个条目其实只有 77 个不同凭据**：
...
| | 优化前 | 优化后 | 省 |
|---|---|---|---|
| 待探测单元 | 175 条目 | **77 凭据** | 56% |
| 单凭据请求数 | 57 次 | **30 次** | 47% |
| 总请求数（最坏） | 9,975 | **2,310** | **77%** |
