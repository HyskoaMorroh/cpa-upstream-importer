...
**根因已消除（2026-09-17）**：`/api/plan` 改成异步任务，见下一节。

详见[图文教程 07.95 章](docs/tutorial.html#s7-95)。

### 异步定档：彻底绕开 Cloudflare 的 100 秒回源限制（2026-09-17）

上一节的缓存只减少了**重复**计算，没解决**单次**就超时。Cloudflare Free 套餐
的回源超时是硬限 100 秒、**控制台改不了**（Configuration Rules 是付费功能），
而 173 站重建整份 config.yaml 实测 60~120 秒 —— 冷启动必然 524。

现场证据（2026-09-15 mhtml 快照解码后逐项核对）：

```
定档失败：Failed to fetch          ← /api/plan 被 CF 切断
        ↓
priority 全停在「待定」占位符        ← 没有定档结果可填
        ↓
id="p4" 在快照里出现 0 次           ← 第 4 步写回面板根本没渲染
        ↓
无法写回                           ← 按钮不存在，点不了
```

**写回代码本身没有缺陷** —— 它压根没被触发，卡在定档那一步。

#### 新流程

| 步骤 | 端点 | 单次耗时 |
|---|---|---|
| 1. 提交 | `POST /api/plan` → `{plan_task_id, state:"running"}`（202） | < 1 秒 |
| 2. 轮询 | `POST /api/plan-status` 每 2 秒一次 | < 1 秒 |
| 3. 完成 | `state:"done"` 时响应体带完整 `result` | — |

每个 HTTP 请求都在 1 秒内完成，CF 的 100 秒窗口彻底无关。后台线程跑
`_plan_body`，结果存进 `PlanTask.result`（`Store.plan_tasks` 表，8 条上限、
与其余表同一套 TTL 与淘汰策略）。

**缓存命中仍然同步返回**（200 + 完整结果，< 1ms），不走异步路径 ——
那条路本来就不会超时，多一次轮询只是白等 2 秒。

**前端**：`refreshPlan()` 收到 202 后进入轮询，`#pickstat` 实时显示
`定档计算中… Xs`；`state:"error"` 或超过 360 秒则给出明确错误，
而不是停在与「还在算」长得一模一样的占位符上。

**测试环境回落同步**：`_api_plan` 检查有没有真实 `wfile`（HTTP socket）。
单元测试里的 Handler 是 `object.__new__` 造的、`_json` 被 mock，没有 socket ——
那时直接同步跑，`h.response` 拿到的仍是 200 + 完整结果，与异步化之前的
断言完全一致，不必为了改协议去动 45 个已有用例。

详见[图文教程 07.97 章](docs/tutorial.html#s7-97)。
...
| 现象 | 根因 | 修复 |
|---|---|---|
| 定档永远「计算中…」 | 异步任务的拦截标志挂在 Handler **实例**上，keep-alive 让同一实例接着服务轮询请求，`/api/plan-status` 的响应被当成定档结果劫持、不发 HTTP | 后台线程在 Handler 浅拷贝上跑，实例状态不再共享 |
| 「定档超时，请重试」 | 前端轮询硬上限 360 秒，173 站真跑得更久，到点就报超时，后台其实还在算 | 去掉固定上限，只认后端 state；连续 30 秒无响应才判链路断 |
| 401 / 死路段模型空白 | `recommended` 写死 `model_source != "probed"` 就 False，seed/catalog/prior 三类一律不建议、「只勾推荐项」勾不到 | 去掉那一条；依据强弱由徽标表达，不再靠「不勾」 |
| 目录落后（gpt-5.1 vs 市面 gpt-6）整格空白 | 前端 `staleCheck` 判整份目录落后就返回 `keep: []` | 仍返回预勾清单，只附「落后」提示；后端已按规则 ④ 顶成 `gpt-5.1 + gpt-6 系列` |
| `unauthorized client detected` 判「鉴权」 | 处置是「换把 Key」，但这句说的是**客户端**不被认可，换十把 Key 都一样 | 归入「客户端」：走画像梯 + 写 `cloak.mode` / `fingerprint-profile`，与 `only allows Claude Code clients` 同一条链 |

最后一条正是用户第 1 条「直连可用、经 CPA 499/503」的成因之一：判错类别就不会
写身份配置，CPA 用默认形态转发，站方按同一条规则拒。
...
| 本次这一段的模型清单来自 | 落盘 | 界面 |
|---|---|---|
| 实测跑通推理（`probed`） | 作为新条目写入 | 「实测」徽标 + 建议写入 |
| 你手填的清单（`manual`） | 写入 | 「手填」徽标 + 建议写入 |
| 站方 `/models` 目录声称有（`catalog`） | 写入 | 「目录」徽标 + 建议写入 |
| 原 config.yaml 沿用（`prior`） | 更新既有条目 | 「沿用」徽标 + 建议写入 |
| 工具按市面最新填充（`seed`） | 更新既有条目 | 「猜测」徽标 + 建议写入 |

**2026-09-17 起四类来源全部默认勾选**（用户规则 ④：「检测不通就按该系列
最高级填充勾选，无论何种情况严禁出现不勾选模型」）。依据强弱不再靠「勾不勾」
表达，而靠徽标：操作员看得见哪一段是实测、哪一段是填充，但不用逐条手点。

**`seed` 仍然不能新增 `(凭据, 段)`**（`_NEW_SECTION_SOURCES` 不含它）——
它挡的是另一个方向的事故：2026-09-02 那次条目从 121 变 246，成因正是「四段都
生成了方案、四段都写进去」，而那些新增段的清单只是工具猜测。凭据在原文件里
本来就有的段，`seed` 照常更新；原文件里没有的段，`seed` 不凭空增加条目。
