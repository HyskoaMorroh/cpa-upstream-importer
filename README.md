...
☑ 全量重探模式
    ...
    （headers/代理/优先级/前缀全部更新）。预计耗时 5-8 分钟。

  站级并发数 [30]
    同时探测的站数量（4U24G VPS 推荐 30-40）

⚠️ 全量重探将重新探测所有既有站。
   config.yaml 中有 175 个既有站。
   此操作会生成完整的 diff 预览，您可以在写回前逐项确认。
```

点「开始探测」时会再确认一次，说明将要探测多少个站、大约多久。

### 耗时与并发

单站四段全通、不开上下文探测约 32 次请求；全不通需要走画像梯救援约 60 次。
串行跑 175 个站要一个多小时，所以站级并发是必须的：
...
```
[12.3s] 提取到 175 个既有站
[12.4s] 总计 176 个站（既有 175 + 新增 1）
[12.5s] 开始批量探测（30 站并发）
[45.2s] 探测进度：42/176 · 成功 12 · 部分通 25 · 失败 5
...
[4m38s] 探测完成：58 成功，89 部分通，29 失败
```
...
全量重探目前只在假上游和 6 项单元测试上验证过，**没有对 175 个真实站跑过一次
